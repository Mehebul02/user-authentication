...
const SignUp = () => {
  ...
  const validate = () => {
    ...
    if (!name.trim()) newErrors.username = 'Name is required';
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setFormData({ usernamename: '', email: '', shopName: '', password: '' });
      }
    } catch (err:any) {
    ...
    <div className="max-w-md mx-auto mt-10 p-6 bg-white shadow-md rounded-xl">
      ...
      <form onSubmit={handleSignup} className="space-y-4">
        <div>
          ...
          />
          {errors.name && <p className="text-sm text-red-500 mt-1">{errors.username}</p>}
        </div>
